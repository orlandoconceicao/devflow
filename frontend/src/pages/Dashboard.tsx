...
import { useDashboard } from '../features/work/hooks';
import { formatDate } from '../utils/format';
export function Dashboard() {
  const { user } = useAuth();
  const query = useDashboard();
  if (query.isLoading) return <LoadingState />;
  if (!query.data) return <ErrorState message="Não foi possível carregar o dashboard." />;
  const data = query.data;
  const stats = [
    ['Projetos ativos', String(data.active_projects), BriefcaseBusiness],
    ['Tarefas pendentes', String(data.pending_tasks), CheckSquare],
    ['Horas este mês', `${data.hours_this_month}h`, Clock3],
    ['Receita do mês', 'R$ 0,00', WalletCards],
  ] as const;
  return (
    <>
          ...
          {data.recent_projects.length ? (
            data.recent_projects.map((p) => (
              <Link className="project-row" to={`/projects/${p.id}`} key={p.id}>
                <span>
                  <b>{p.name}</b>
                  <small>{p.client_detail.name}</small>
                </span>
            ...
            <EmptyState
              ...
              description="Seus projetos recentes aparecerão aqui."
            />
          )}
        </section>
        <section className="panel">
          ...
          {data.upcoming_deadlines.length ? (
            data.upcoming_deadlines.map((p) => (
              <Link className="deadline-row" to={`/projects/${p.id}`} key={p.id}>
                <span>
                  <b>{p.name}</b>
                  <small>{p.client_detail.name}</small>
                </span>
                <time>{formatDate(p.due_date)}</time>
              </Link>
            ))
          ) : (
            <EmptyState
              title="Tudo tranquilo por aqui"
              description="Você ainda não possui prazos próximos."
            />
          )}
        </section>
      </div>
      <section className="panel section-gap">
        ...
        {data.recent_activity.length ? (
          ...
          <EmptyState
            ...
            description="As atualizações do seu workspace aparecerão aqui."
          />
        )}
      </section>
