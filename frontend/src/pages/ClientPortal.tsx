    ...
    <>
      <div className="page-head">
        <div>
          ...
          <p>Acompanhe seus projetos e aprove entregas.</p>
        </div>
      </div>
      <div className="stats">
        <article>
          <div>
            ...
            <strong>{d.data?.active_projects || 0}</strong>
          </div>
        </article>
        <article>
          <div>
            <span>Aguardando aprovação</span>
            <strong>{d.data?.pending_deliverables || 0}</strong>
          </div>
        </article>
      </div>
      <section className="panel section-gap">
        ...
        {d.data?.projects.length ? (
          ...
          <EmptyState title="Nenhum projeto compartilhado" />
        )}
      </section>
    </>
  );
...
export function ClientProject() {
  ...
  const d = useQuery({
    ...
    queryFn: () =>
      api
        .get<PaginatedResponse<Deliverable>>('/deliverables/', { params: { project: id } })
        .then((r) => r.data),
  });
  return (
    <>
      <div className="page-head">
        <div>
          ...
          <p>Revise os materiais compartilhados.</p>
        </div>
      </div>
      {d.isLoading ? (
        <LoadingState />
      ) : d.data?.results.length ? (
        d.data.results.map((x) => (
          <section className="panel section-gap" key={x.id}>
            ...
            {x.status === 'READY_FOR_REVIEW' && (
              <div className="actions section-gap">
                <button
                  className="button"
                  onClick={async () => {
                    await api.post(`/deliverables/${x.id}/approve/`);
                    d.refetch();
                  }}
                >
                  Aprovar
                ...
                <button
                  ...
                  onClick={async () => {
                    const message = prompt('Quais alterações são necessárias?');
                    if (message) {
                      await api.post(`/deliverables/${x.id}/request-changes/`, { message });
                      d.refetch();
                    }
                  }}
                >
